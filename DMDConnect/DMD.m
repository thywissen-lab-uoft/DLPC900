    ...
    methods
        ...
        function definePattern(dmd,opts) % 0x1A34
            ...
            if nargin==1            
                ...
                bitDepth=opts.bitDepth;    % desired bit depth (1 corresponds to bitdepth of 1)
                leds=opts.leds;    % select which color to use
                triggerIn=opts.triggerIn;    % wait for trigger or cuntinue
                darkTime=opts.darkTime;    % dark time after exposure in us
                triggerOut=opts.triggerOut;    % use trigger2 as output
                patternIdx=opts.patternIdx;    % image pattern index
